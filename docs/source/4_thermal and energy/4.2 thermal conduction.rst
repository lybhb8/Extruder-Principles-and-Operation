...
其中 :math:`\pmb q` 是热流率（W）， :math:`k` 是热导率 :math:`(\mathrm{W\,m^{-1}\,K^{-1}})` ， :math:`\pmb{A}` 是导热的横截面 :math:`(\mathrm{m}^{2})` ， :math:`-\mathrm{d}T/\mathrm{d}x` 是热流方向上的温度梯度 :math:`(\mathrm{K\,m^{-1}})` ，负号表示热量沿温度下降的方向流动。
...
使得总温差 :math:`T_{1}-T_{2}` 被细分为与相应的厚度 :math:`\mathrm{d}x_{1},\mathrm{d}x_{2}` 成比例并且与电导率成反比。 

如果热量从筒式加热器通过 :math:`30\,\mathrm{mm}` 厚的筒体（钢的 :math:`\boldsymbol{k}` 为 :math:`50\,\mathrm{W\,m^{-1}\,K^{-1}}` ）和5毫米厚的LDPE（ :math:`5\,\mathrm{mm}` ）传导，总温差为 :math:`100^{\circ}\mathbf{C}` ，则筒体和聚合物之间的温差分别为 :math:`5.7^{\circ}\mathbf{C}` 和 :math:`94.3^{\circ}\mathbf{C}_{\mathrm{{i}}}`。
...
.. math::
   ...
   {\frac{\mathrm{d}T}{\mathrm{d}t}}=\alpha\,{\frac{\mathrm{d}^{2}T}{\mathrm{d}x^{2}}}\tag{4.7}


其中 :math:`t` 是时间， :math:`x` 是热传导方向上的距离， 热扩散率（Thermal Diffusivity） :math:`\alpha={k}/{\rho C_{\mathfrak{p}}}` :math:`(\mathrm{m}^{2}\,\mathrm{s}^{-1})` .


一种常见的情况是，处于均匀温度的 :math:`T_{\mathfrak{b}}` 固体突然与处于均匀恒定温度 :math:`T_{\mathrm{a}}` 的周围环境接触。
...
（摘自McAdams 1954，方程式（3.4））。应该注意的是，在这种情况下，固体的表面温度瞬间上升到周围环境的 :math:`T_{\mathrm{a}}` ，
向中心的上升逐渐变小和变慢。
...
其中 :math:`W` 是辐射功率 :math:`(\mathrm{W\,m^{-2}})` ， :math:`T` 是绝对温度（K）和Stefan-Boltzmann常数 :math:`\sigma=\bar{5.67}\times10^{-8}\,\mathrm{W\,m^{-2}\,K^{-4}}` 。
