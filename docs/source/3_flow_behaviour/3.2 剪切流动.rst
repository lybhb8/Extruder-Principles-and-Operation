...
综合:
        ...
        :math:`\int_{0}^{h}\mathrm{d}V=-\frac{1}{\eta}\cdot\frac{\mathrm{d}P}{\mathrm{d}L}\int_{0}^{h}y\,\mathrm{d}y` ``-                      (3.28)``
...
在距离中心 :math:`\pmb{h}` 处，厚度为 :math:`\mathrm{d}h`
的薄片中的体积流量为：
...
:math:`\mathrm{d}P={\frac{12\eta Q\,\mathrm{d}L}{T H^{3}}}` ``-                      (3.34)``
...
:math:`\mathrm{d}P/\mathrm{d}L`\ 恒定且等于 :math:`P/L`\ 。公式 (3.22)
可以重写：
...
，则相应的值分别为 :math:`16.6\%` 和 :math:`8.5\%`\ （对于
...
:math:`w+\mathrm{d}w` 的相邻层之间的界面上的剪切力为：
...
.. math::
   ...
   F t[(w+\mathrm{d}w)-w]=\tau x z\,t\,\,\mathrm{d}w\tag{3.54}
...
.. math::
   ...
   {\mathrm{Strain~energy~per~unit~volume}}={\frac{\eta W^{2}}{h^{2}}}\,\mathrm{W}\,\mathrm{m}^{-3}\tag{3.58}
...
.. math::
   ...
   {\mathrm{Strain~energy~per~unit~volume}}={\frac{\eta W^{2}}{h^{2}}}\,\mathrm{W}\,\mathrm{m}^{-3}(\mathrm{simple~shear})\tag{3.59}
...
其中 :math:`\mathrm{d}P` 是长度 :math:`L` 上的压降。
...
其中\ :math:`\mathrm{d}P`\ 是长度\ :math:`\pmb{L}`\ 的压降。

由方程（3.65）给出的宽狭缝中局部应变能的分布也绘制在图3.9中。下面的例子说明了应变能量和粘性加热的数量级。在直径为\ :math:`100\,\mathrm{mm}`\ 的挤出机螺杆中，通道宽度为\ :math:`100\,\mathrm{mm}`\ ，转速为\ :math:`1\,\mathrm{rps},`\ 速度约为\ :math:`0.3\,\mathrm{m}\,\mathrm{s}^{-1}`
...
.. math::
   ...
   \begin{array}{r l}{\mathrm{Strain\ energy}=\eta\stackrel{.}{\gamma}^{2}}&{}\\ {=10^{3}\times30^{2}}\\ {}&{=9\times10^{5}\,\mathrm{W\,m}^{-3}}\end{array}
...
根据表4.1，密度和比热为 :math:`750\,\mathrm{kg}\,\mathrm{m}^{-3}` and
...
在毛细管中，假设一个直径为\ :math:`5\,\mathrm{mm}`\ 、长为\ :math:`10\,\mathrm{mm}`\ 的模头，并测量流速。\ :math:`Q^{-}\,{=}\,0.1\times\,\mathrm{i}0^{-4}\,\mathrm{m}^{3}\,\mathrm{s}^{-1}`\ ，牛顿粘度为\ :math:`10^{3}\,\mathrm{N}\,\mathrm{s}\,\mathrm{m}^{-2}`
...
的模头入口——一个尖锐的锥形，以夸大纵向分量。显然，由于平均剪切速率较低，锥形入口的剪切应变能将小于毛细管。现在
...
.. math::
   ...
   \begin{array}{c}{{\Lambda\dot{\varepsilon}^{2}=3\times10^{3}\times3.518^{2}}}\\ {{=37.1\times10^{3}\,\mathrm{W\,m}^{-3}}}\end{array}

请注意，模头中的剪切应变能约为该拉伸能的\ :math:`10^{4}`\ 倍。存储在锥度中的拉伸能为：

.. math::


   37.1\times10^{3}\times13.09\times10^{-6}=0.486\,\mathrm{W}

大部分能量以弹性方式储存，并在模头出口处通过弹性恢复释放，从而在长度上产生横向膨胀和收缩。然而，如果这种能量通过平行通道中的内摩擦消散，例如在非常长的模头中，温度上升的速度将为
